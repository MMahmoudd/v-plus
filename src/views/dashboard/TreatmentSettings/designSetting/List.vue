  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/designSettingForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/designSettingForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  const designSettingService = ServiceFactory.get('designSetting')
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await designSettingService.getAllItems(itemsPerPage, page, pageNumber)
        items.data.map(item => {
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await designSettingService.deleteOneItem(userDetails.id)
