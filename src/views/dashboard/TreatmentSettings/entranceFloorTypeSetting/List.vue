  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/entranceFloorTypeSettingForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/entranceFloorTypeSettingForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  const SettingService = ServiceFactory.get('entranceFloorTypeSetting')
