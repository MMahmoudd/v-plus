    ...
    <v-card>
      <v-card-title>
        ...
        <router-link
          ...
          :to="{ path: '/treatment-settings/regionsForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        >
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/regionsForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      disabled: false,
      headers: [
        { text: 'إسم المنطقة', sortable: true, value: 'name' },
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await RegionsService.getAllItems(itemsPerPage, page, pageNumber)
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await RegionsService.deleteOneItem(userDetails.id)
