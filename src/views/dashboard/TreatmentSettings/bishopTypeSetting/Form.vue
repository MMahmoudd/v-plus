...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async newItem (data) {
        const item = await bishopTypeSettingService.addOneItem(data)
        if (item.success) {
          this.successMessage = 'تمت الاضافة بنجاح'
          this.successSnackbar = true
          setTimeout(() => {
            this.$router.push('/treatment-settings/bishopTypeSetting')
          }, 1500)
        } else {
      ...
      async updateContent (id, data) {
        const item = await bishopTypeSettingService.updateOneItem(id, data)
        if (item.success) {
      ...
      async fetchOneItem (id) {
        ...
        const user = await bishopTypeSettingService.fetchOneItem(id)
        console.log(user)
        if (user.data.status) {
