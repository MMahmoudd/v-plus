  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/bishopTypeSettingForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/bishopTypeSettingForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  const bishopTypeSettingService = ServiceFactory.get('bishopTypeSetting')
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await bishopTypeSettingService.getAllItems(itemsPerPage, page, pageNumber)
        items.data.map(item => {
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await bishopTypeSettingService.deleteOneItem(userDetails.id)
