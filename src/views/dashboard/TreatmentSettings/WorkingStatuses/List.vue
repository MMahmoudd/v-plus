    ...
    <v-card>
      <v-card-title>
        ...
        <router-link
          ...
          :to="{ path: '/treatment-settings/WorkingStatusesForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        >
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/WorkingStatusesForm/' + item.id"
              >
                <v-btn
                  small
                  fab
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await WorkingStatusesService.getAllItems(itemsPerPage, page, pageNumber)
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await WorkingStatusesService.deleteOneItem(userDetails.id)
