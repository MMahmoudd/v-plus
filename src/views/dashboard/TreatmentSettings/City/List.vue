  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/citiesForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/citiesForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      disabled: false,
      headers: [
        { text: 'إسم المدينة', sortable: true, value: 'name' },
        { text: 'إسم المنطقة', sortable: true, value: 'region' },
