  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/propertyTypesForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/propertyTypesForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      disabled: false,
      headers: [
        { text: 'إسم النوع', sortable: true, value: 'name' },
    ...
    methods: {
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await PropertyTypesService.deleteOneItem(userDetails.id)
