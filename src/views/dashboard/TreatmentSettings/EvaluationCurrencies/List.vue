  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/EvaluationCurrenciesForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/EvaluationCurrenciesForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      disabled: false,
      headers: [
        { text: 'إسم العملة', sortable: true, value: 'name' },
        { text: 'الرمز', sortable: true, value: 'symbol' },
    ...
    methods: {
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await EvaluationCurrenciesService.deleteOneItem(userDetails.id)
