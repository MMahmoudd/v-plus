<template>
  ...
  >
    <v-card class="py-5">
      <v-card-title>
        {{ this.$route.params.id ? 'تعديل العملة' : 'اضافة عملة' }}
      </v-card-title>
      <template>
        <v-form
          v-model="valid"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async newItem (data) {
        const item = await EvaluationCurrenciesService.addOneItem(data)
        if (item.success === true) {
          this.successMessage = 'تمت الاضافة بنجاح'
          this.successSnackbar = true
          setTimeout(() => {
            this.$router.push('/treatment-settings/EvaluationCurrencies')
          }, 1500)
        } else {
      ...
      async updateContent (id, data) {
        const item = await EvaluationCurrenciesService.updateOneItem(id, data)
        if (item.success === true) {
      ...
      async fetchOneItem (id) {
        ...
        const user = await EvaluationCurrenciesService.fetchOneItem(id)
        console.log(user)
        this.data = user.data
