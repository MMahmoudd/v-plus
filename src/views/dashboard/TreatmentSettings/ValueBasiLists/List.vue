  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <router-link
          :to="{ path: '/treatment-settings/ValueBasiListsForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/ValueBasiListsForm/' + item.id"
              >
                <v-btn
                  small
                  fab
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await ValueBasiListsService.getAllItems(itemsPerPage, page, pageNumber)
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await ValueBasiListsService.deleteOneItem(userDetails.id)
