    ...
    <v-card>
      <v-card-title>
        ...
        <router-link
          ...
          :to="{ path: '/treatment-settings/propertyRatingsForm/'}"
          color="blue"
        >
          <v-btn
            class="mx-2"
        ...
        >
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <router-link
                :to="'/treatment-settings/propertyRatingsForm/' + item.id"
              >
                <v-btn
                  small
                  fab
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      disabled: false,
      headers: [
        { text: 'إسم التصنيف', sortable: true, value: 'name' },
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const items = await PropertyRatingsService.getAllItems(itemsPerPage, page, pageNumber)
      ...
      async deleteUser (userDetails) {
        this.loading = true
        this.disabled = true
        const deleteUsers = await PropertyRatingsService.deleteOneItem(userDetails.id)
