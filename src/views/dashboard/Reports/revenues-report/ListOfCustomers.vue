        ...
        <table>
          ...
          <tbody>
            ...
            <tr
              ...
              :key="index"
            >
              <td data-prefix="رقم المعاملة">
                <router-link :to="'/Evaluate-Treatment/'+item.id">
                  {{ item.transaction_id }}
                </router-link>
              </td>
              <td data-prefix="العميل">
              ...
              <td data-prefix="المدينة">
                {{ item.city.name }}
              </td>
              <td data-prefix="الحي">
                {{ item.neighborhood.name }}
              </td>
              <td data-prefix="نوع العقار">
                {{ item.propertytype.name }}
              </td>
              <td data-prefix="استخدام العقار">
                {{ '1' }}
              </td>
              <td data-prefix="السعر">
                {{ item.trans_report_price || 0 }}
              </td>
              <!-- <td
            ...
            <!--TOTAL-->
            <!-- <tr>
              <td class="total-in-mobile">الإجمالي</td>
              <td
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.fetchAll(this.customerId, { page: this.page.current })
    },
    methods: {
      async fetchAll (customerId, options) {
        this.loading.list = true
        const { data: { data: list, total, next_page: nextPage } } = await revenuesReportService.getAllItemsByCustomerId(customerId, options)
