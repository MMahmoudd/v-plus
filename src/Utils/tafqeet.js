...
const _tafqeet = value => {
    if (String(value).includes('.')) {
        const parts = String(value).split('.')
        return 'فقط ' + tafqeet(+parts[0]) + ' ريال سعودي و ' + tafqeet(parts[1]) + ' هللة لا غير'
    } else {
       return 'فقط ' + tafqeet(value) + ' ريال سعودي لا غير'
    }
}
export default _tafqeet
